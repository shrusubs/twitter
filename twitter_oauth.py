...
# We have to start by obtaining a 'request' token --> temporary credentials from the server
...
# create an oauth session to pass in client key and client secret
...
# Taking ^ baseurl and ask to authorize
...
tweets_list = json.loads(r.text)

for each_tweet in tweets_list["statuses"]:
